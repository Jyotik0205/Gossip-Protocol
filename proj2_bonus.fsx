...
let mutable input4=fsi.CommandLineArgs.[4]|> int;
...
let r = System.Random()
let nums = r.GetValues(1, input1) |> Seq.take input4
let FailureTopology = new System.Collections.Generic.List<int>()                                               //Topology of the actor 
for x in nums do
  FailureTopology.Add(x)
             ...
             let rec loop()=
                actor{
                   ...
                   |CalcTopology->          let inp=getnearestsquare input1
                                            ...
                                            if (n%size)<>0 && not(FailureTopology.Contains(n+1))  then 
                                                Topology.Add(n+1)
                                            if (n-size)>0 && not(FailureTopology.Contains(n-size)) then
                                               Topology.Add(n-size)
                                            if (n+size)<input1 && not(FailureTopology.Contains(n+size)) then
                                                Topology.Add(n+size)
                                            if(n%size)<>1 && not(FailureTopology.Contains(n-1)) then
                                               Topology.Add(n-1)

                                            if input2="imp2D" then
                                                let mutable next=0
                                                next<-rand.Next(1,inp+1)
                                                while next=n || Topology.Contains(next)||FailureTopology.Contains(next) do
                                                        next<-rand.Next(1,inp+1)
                                                Topology.Add(next)
                                            let parent=system.ActorSelection("akka://example3/user/parent")
                                            parent.Tell(Tdone)
                   ...
                   |CalcTopologyLFull->     if input2="line" then
                                                 if n=1 && not(FailureTopology.Contains(2)) then
                                                        Topology.Add(2)
                                                    elif n=input1 && not(FailureTopology.Contains(input1-1)) then
                                                        Topology.Add(input1-1)
                                                    else
                                                        if not(FailureTopology.Contains(n-1)) then
                                                          Topology.Add(n-1)
                                                        if not(FailureTopology.Contains(n+1)) then
                                                          Topology.Add(n+1)
                                                  ...
                                                  for i in 1 .. input1 do
                                                    if n<>i &&  not(FailureTopology.Contains(i)) then 
                                                        Topology.Add(i) 
                                            let parent=system.ActorSelection("akka://example3/user/parent")
                                            parent.Tell(Tdone)
...
let actor= spawn system "parent" <| fun mailbox->
               ...
               let rec loop()=                                                         
                  ...
                  // printfn "From Parent %A" msg
                   match msg with
                   |Start->         let mutable k=input1/2;                                                      //Start the process
                                    while FailureTopology.Contains(k) do
                                        k<-k+1                                                       
                                    if input3="push-sum" then
                                        Actor.[k].Tell(SumRumor (0.0,0.0,0))
                                    else
                                        Actor.[k].Tell(Rumor (3,0))
                   ...
                   |Done (n,k)->    nActorsDone<-nActorsDone+1                             //Convergence Controller 
                                    ...
                                    if started=nActorsDone && started<>0 && nActorsDone<>input1 && running<=0 then
                                     printfn "Actors convergeres= %d out of %d but only able to reach %d actors" converged input1 started
                                     flag<-true
